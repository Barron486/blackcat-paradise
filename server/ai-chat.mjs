...
const TOPICS=[
  ...
  {id:'roleplay',name:'角色互動',description:'以各自的個性回應當下對話，不演說或朗讀設定。'},
  {id:'custom',name:'自訂內容',description:'依 GM 提供的話題或活動說明發言。'},
];
const DEFAULT={enabled:false,provider:'codex',model:MODELS.codex,ollamaUrl:DEFAULT_OLLAMA_URL,ollamaModel:DEFAULT_OLLAMA_MODEL,topic:'casual',customPrompt:'',intervalSeconds:90,dailyMessageLimit:0,respondToPlayers:true,ambientChat:false,speakers:[],personas:{}};
...
export class AiChatService {
  constructor(service,{now=Date.now,apiKey=process.env.OPENAI_API_KEY||'',fetchImpl=globalThis.fetch}={}){
    ...
    `);
    if(!this.db.prepare('PRAGMA table_info(ai_chat_jobs)').all().some(c=>c.name==='reply_kind'))this.db.exec("ALTER TABLE ai_chat_jobs ADD COLUMN reply_kind TEXT NOT NULL DEFAULT ''");
  ...
  speakers(){
    return this.db.prepare("SELECT a.id,a.username,s.data,l.slot,l.display_name,l.map_name,l.expires_at FROM accounts a JOIN saves s ON s.account_id=a.id LEFT JOIN leases l ON l.account_id=a.id WHERE a.role='player' ORDER BY a.username").all().flatMap(row=>{
      ...
      if(!doc?.p?.cls)return [];
      const speaker={id:row.id,username:row.username,name:doc.p.name||row.username,cls:doc.p.cls,online:row.expires_at>this.now()};
      return [{...speaker,defaultPersonality:defaultPersonality(speaker)}];
    });
  }
  status(user){
  ...
  update(user,body){
    ...
    requireValue(Array.isArray(body.speakers)&&body.speakers.every(id=>typeof id==='string'),'發言角色名單格式不正確');
    ...
    const ambientChat=body.ambientChat??this.settings().ambientChat;
    requireValue(typeof ambientChat==='boolean','主動閒聊必須為開關值');
    const proposed=body.personas??this.settings().personas;
    requireValue(proposed&&typeof proposed==='object'&&!Array.isArray(proposed),'角色個性格式不正確');
    const personas={};
    for(const [id,value] of Object.entries(proposed)){
      if(!available.has(id)&&body.personas===undefined)continue;
      requireValue(available.has(id),'個性設定必須屬於已創角的一般玩家');
      requireValue(typeof value==='string'&&value.length<=500&&!/[\x00-\x08\x0b\x0c\x0e-\x1f]/.test(value),'每位角色個性最多 500 字');
      if(value.trim())personas[id]=value.trim();
    }
  ...
  finish(body,provider){
    requireValue(typeof body.text==='string'&&body.text.length<=240&&!/[\x00-\x08\x0b\x0c\x0e-\x1f]/.test(body.text),'AI 訊息最多 240 字');
    return this.service.transaction(()=>{
