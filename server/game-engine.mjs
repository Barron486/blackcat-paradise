...
export class HeadlessGame {
  constructor({values = {}, slot = 1} = {}) {
    ...
    w.__cliLog = (type,message) => {
      const fragment=w.document.createElement('template');fragment.innerHTML=String(message);
      const text=fragment.content.textContent;
      // Keep equipment rarity colours, but no handlers, links, images or executable markup.
      for(const el of [...fragment.content.querySelectorAll('*')]){
        if(!['SPAN','B','STRONG','EM','I','BR'].includes(el.tagName)){el.replaceWith(w.document.createTextNode(el.textContent));continue;}
        const cls=el.className,color=el.style.color;
        for(const attr of [...el.attributes])el.removeAttribute(attr.name);
        if(typeof cls==='string'&&/^[a-zA-Z0-9 _:-]*$/.test(cls))el.className=cls;
        if(color)el.style.color=color;
      }
      this.logs.push({id:++this.logId,at:Date.now(),tick:this.run('state.ticks'),type,message:text,html:fragment.innerHTML});
    ...
    this.run(`for(const name of __args) { if(typeof window[name] === 'function') window[name] = function(){}; }
      ...
      // Account leases replace the browser's cross-tab localStorage registry.
      _roleSessionHeartbeat = function(){};
      _roleOtherActiveSessions = function(){return [];};
  ...
  create({classId,name,allocation = {},gender = 'male',classicMode = false} = {}) {
  ...
  load(slot = this.slot) {
    ...
    const doc=this.decodeSave(this.window.localStorage.getItem('lineage_idle_save_'+this.slot));
...
function validateSlot(slot) {
  if(!Number.isInteger(slot) || slot < 1 || slot > 8) throw new Error('存檔欄位須為 1–8');
  return slot;
}
