...
export class MonsterSettings {
  constructor(world){this.world=world;this.rows=world.catalog.creatures||[];this.byId=new Map(this.rows.map(m=>[m.id,m]));}
  list(user,params){
    ...
    return {revision:state.revision,strength:state.monsterStrength,broadcast:state.killBroadcast,announcement:state.announcement,total:rows.length,totalPages,page,ids:rows.map(m=>m.id),maps:this.world.catalog.maps,
      rows:rows.slice((page-1)*size,page*size).map(m=>({...m,rule:state.monsters[m.id]||{},effective:GameMonsterRules.apply(m.data,state.monsters[m.id],state.monsterStrength),fields:GameMonsterRules.fields(m.data),broadcast:state.killBroadcast.monsters.includes(m.id)}))};
  }
  command(body){
    ...
    check(command.reset||command.strength!==undefined||command.broadcast!==undefined||Object.keys(command.values||{}).length,'沒有選擇要修改的項目');
    return command;
  }
  apply(state,command){
    ...
    for(const id of command.ids){
      const rule=command.reset?{}:structuredClone(state.monsters[id]||{});
