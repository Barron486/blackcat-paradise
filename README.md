...
- **藍鑽商店**：由 GM 發放帳號共用藍鑽；更名卡 3,000 藍鑽、更改密碼卡 500 藍鑽、全狀態 300 藍鑽／1 小時、偷窺卡 300 藍鑽／次。偷窺卡可從線上名單選取角色，取得裝備、HP／MP、等級、正義值及地圖的雲端快照。全狀態套用至選定角色，重複購買延長時間、離線照常倒數。線上版匯出為唯讀角色報告，停用角色匯入及免費更名。
- **玩家交易所**：玩家以藍鑽或金幣上架物品，成交收取 20% 系統手續費（不足一枚進位）；支援物品託管、購買、下架與金幣收入領取。
- **聊天與通知**：世界聊天、線上人數及名單、稀有裝備掉落跑馬燈與聊天室公告，包含台灣時間的掉落時間戳記。GM 在「世界設定 → 稀有掉落廣播」可開關並複選傳說、遺物、極低掉率裝備；不補播停用期間的掉落，原聊天公告保留。GM 可管理 AI 聊天內容、角色、回覆玩家與生成頻率，支援本機 Ollama、Codex CLI 或伺服器模型 API。玩家聊天保存發言時的角色 ID（角色名稱），公開名單與訊息不包含登入帳號；NPC 名字可點擊感謝、嘲諷或私訊。線上名單只顯示總人數；位置預設限自己、GM 與同血盟成員，GM 可公開位置並指派跨玩家血盟名冊。玩家介面統一顯示角色名稱及在線總人數；模型資訊與生成紀錄僅供 GM 管理。
- **掉落配色**：本圖掉落清單、掉寶日誌及公告使用傳說橙色、遺物藍色、其餘極低掉率裝備紫色。極低掉率按該怪物 GM 設定的基礎機率大於 0 且不高於 0.01% 判定，不受世界倍率與個人加成影響；只涵蓋武器、防具、飾品，排除箭矢與消耗材料。
...
- **自動喝水**：三條獨立啟用的血量／藥水規則，可設定紅水、橙水、白水。符合條件時優先套用最低血量門檻，同門檻依規則順序，每次只喝一瓶並共用一秒冷卻；缺藥時依自動購買設定補至 100 瓶，無法補貨則使用下一條符合規則。原喝水設定保留在規則 1，新增規則預設關閉；設定隨角色儲存。
...
本版 **188 項測試通過**，涵蓋帳號隔離、CSRF、GM 權限、存檔衝突、升級保存、選角與創角期間的存檔隔離、藍鑽與卡片、交易重試及原子性、自動販賣複選、三條喝水規則與存檔相容、CLI、AI 聊天、世界設定、裝備配色、廣播範圍與開關、掉落時間及聊天室公告、角色聊天名稱、NPC 互動、偷窺卡扣款與重試，以及位置公開權限與 GM 血盟名冊。測試使用隔離資料庫。
