...
- **背景掛機**：戰鬥、經驗、金幣與掉寶由伺服器持續結算；關閉分頁、切換到背景或鎖屏不會停止。每帳號僅目前選取的角色掛機，重新登入接回同一場戰鬥。回到選角、遊戲內登出、暫停或死亡會停止；喝水、補貨與死亡仍沿用原規則。掛機角色及暫停狀態保存在 SQLite，伺服器重啟後自動恢復；伺服器停機期間不補算獎勵。
- **地圖與副本傳送**：傲慢之塔入口與攀塔、時空裂痕入場／撤離／領獎、安塔瑞斯副本與助戰、聖地入口、鑰匙房、隱藏地圖、攻城及競技場均由伺服器驗證與結算。重連保留副本進度及競技場對戰狀態，請求重試不重複扣除入場道具；仍遵守等級、道具、每日次數及 GM 地圖開放設定。
- **存檔驗證**：伺服器拒絕非法數值、道具、強化、初始配點及偽造 GM 狀態；更新失敗時保留原雲端資料，GM 可查看拒絕紀錄。完整範圍與限制見 [存檔安全說明](SAVE-SECURITY.md)。
- **GM 管理**：線上及離線玩家增益、死亡、復活、物品發放、權限管理與操作稽核。可調整經驗、金幣、掉落倍率、地圖開放與進入等級、全體傳送、掉落率顯示與每隻怪物的物品掉落率。
- **怪物管理**：511 種怪物的完整原始資料、技能與出沒地圖；支援搜尋、篩選、個別及批次調整 HP、等級、攻防、速度、經驗、金幣與技能數值。全服／個別強度倍率可設 0.1～20 倍，影響 HP 與傷害；只套用新生成怪物，既有戰鬥保留原狀。可恢復原始能力；修改有版本檢查、確認預覽及稽核紀錄。
- **GM 廣播**：可勾選特定怪物的擊殺跑馬燈與聊天室捷報，包含角色名稱、地圖與台灣時間；預設關閉，須勾選怪物並開啟總開關，多階段頭目只廣播最終擊敗。另可發布 300 字內自訂公告，選擇置頂固定文字或循環跑馬燈，設定 10 秒～7 天持續時間或提前關閉；同時保留一則，不被掉寶／擊殺通知取代。
- **怪物重生間隔**：GM 可個別或批次設定 1 秒～7 天（秒／分鐘／小時）及恢復原本規則。依角色分別記錄最終擊殺時間，換圖與重啟仍保留冷卻；多階段頭目共用設定，軍王／雙頭目／聖地到期才扣重生道具，野外與裂痕保留原遇敵機率。
- **寵物即時狀態**：出戰、收回、換裝與復活後立即同步「隊伍與夥伴」及保管介面；HP／MP／等級／經驗、異常與增益剩餘秒數隨伺服器更新，喝水門檻輸入即保存且保留編輯焦點。
- **傭兵進度與換裝**：隊員經驗、升級及正義值隨伺服器戰鬥存檔即時寫回各角色；回村、重新招募與登入保留經驗。傭兵公會可從隊長背包替隊員穿戴／卸裝；裝備轉移、隊員與隊長存檔在同一交易提交，請求重試不重複消耗。
...
多人功能包括聊天、在線狀態、玩家交易與 GM 指令同步。線上角色的操作由伺服器驗證並執行原版遊戲規則；各帳號仍有自己的戰場，尚未改為真人共享戰場。背景掛機不會延長玩家的連線狀態。
...
本版 **313 項測試通過**，涵蓋帳號隔離、CSRF、GM 權限、存檔衝突、伺服器存檔驗證與拒絕回滾、升級保存、選角與創角期間的存檔隔離、藍鑽與卡片、交易重試及原子性、自動販賣複選、三條喝水規則與隊伍設定保存、寵物即時狀態與設定保存、CLI、AI 聊天、世界設定、裝備配色、廣播範圍與開關、掉落時間及聊天室公告、角色聊天名稱、NPC 互動、偷窺卡扣款與重試、位置公開權限與 GM 血盟名冊、背景掛機與重啟恢復、二次傳送、副本階段、競技場與入場扣款重試、傭兵換裝與即時經驗、怪物批次調整、重生冷卻與重登保存、擊殺捷報及置頂公告到期。測試使用隔離資料庫。
...
`railway.json` 設定 `sleepApplication: false`，保持背景掛機運作。請保持單一副本，避免多個程序同時結算同一個帳號；每個程序最多同時執行 64 個角色，已開始的背景掛機不會因玩家斷線而被移除。
