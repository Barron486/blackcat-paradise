...
- **聊天與通知**：世界聊天、線上人數及名單、稀有裝備掉落跑馬燈與聊天室公告，包含台灣時間的掉落時間戳記。GM 在「世界設定 → 稀有掉落廣播」可開關並複選傳說、遺物、極低掉率裝備；不補播停用期間的掉落，原聊天公告保留。GM 可管理 AI 聊天內容、角色、回覆玩家與生成頻率，支援本機 Ollama、Codex CLI 或伺服器模型 API。玩家聊天保存發言時的角色 ID（角色名稱），公開名單與訊息不包含登入帳號；NPC 名字可點擊感謝、嘲諷或私訊。玩家介面統一顯示角色名稱及在線總人數；模型資訊與生成紀錄僅供 GM 管理。
...
本版 **180 項測試通過**，涵蓋帳號隔離、CSRF、GM 權限、存檔衝突、升級保存、藍鑽與卡片、交易重試及原子性、自動販賣複選、三條喝水規則與存檔相容、CLI、AI 聊天、世界設定、裝備配色、廣播範圍與開關、掉落時間及聊天室公告、角色聊天名稱、NPC 互動及偷窺卡扣款與重試。測試使用隔離資料庫。
