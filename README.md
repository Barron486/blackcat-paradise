...
本版 **303 項測試通過**，涵蓋帳號隔離、CSRF、GM 權限、存檔衝突、伺服器存檔驗證與拒絕回滾、升級保存、選角與創角期間的存檔隔離、藍鑽與卡片、交易重試及原子性、自動販賣複選、三條喝水規則與隊伍設定保存、寵物即時狀態與設定保存、CLI、AI 聊天、世界設定、裝備配色、廣播範圍與開關、掉落時間及聊天室公告、角色聊天名稱、NPC 互動、偷窺卡扣款與重試、位置公開權限與 GM 血盟名冊、背景掛機與重啟恢復、二次傳送、副本階段、競技場與入場扣款重試、傭兵換裝與即時經驗、怪物批次調整、擊殺捷報及置頂公告到期。測試使用隔離資料庫。
