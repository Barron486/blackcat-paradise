...
function _wcDynamicTopic(q) {
    ...
    // ⚠️ 「藍鑽」不是背包物品（會被物品「鑽石」substring 誤中）→ 交給 blackmarket 主題
    if (item && sourceAsked && (!sameMobItemName || mobDropAsked) && !/藍鑽|藍鑽/.test(q)) return { key: 'item-source', gen: function () { return _wcItemSourceAnswers(item.id); } };
...
const WC_TOPICS = [
    ...
    },
    {
        ...
        gen: function () {
            return [
                ...
                '想指定拿就去潘朵拉黑市搜索，100 顆藍鑽一次，未知遺物那個選項還保證是你圖鑑沒有的。',
    ...
    // ===== v3.6.61 依巴哈玩家實際問法補的六個主題 =====
    {
        ...
        gen: function () {
            return [
                ...
                '邪惡狀態噴掉的裝備別放棄：亞丁的聖使阿卡塔會記錄最近遺失的幾件，花藍鑽就能指定贖回其中一件，強化值跟詞綴都照原樣回來。',   // 🕊️ v3.6.86 裝備贖回
    ...
    {
        key: 'blackmarket', kw: ['交易所', '黑市', '潘朵拉', '藍鑽', '藍鑽', '收購', '鑽收', '金幣收購', '收購NPC', '叫賣NPC', '未知遺物', '搜索遺物', '喊收', '吵死了', '驅離', '出價太低', '低價'],
        gen: function () {
            if(window.CloudStore)return ['玩家交易所可上架背包中未上鎖的物品，選藍鑽或金幣，成交收取 20% 手續費。','藍鑽由 GM 儲值，交易售出藍鑽會直接入帳；金幣收入到交易所的「我的交易」領取。'];
            return [
                '黑市在潘朵拉那邊，商品會定時輪換，看到想要的別猶豫，晚五分鐘就被掃光了。',
                '藍鑽主要靠村莊裡喊收購的玩家：把他要的東西給他就換藍鑽，物品放倉庫也算數。',
                '藍鑽存夠可以在黑市收購搜索指定類型的遺物；選「未知遺物」保證出圖鑑還沒收錄的，蛋類遺物只有這個管道。',
                '黑市也能自己出價收購：出得比行情高就會上架，掛著等貨上門就好。',
                '世界頻道喊「鑽收」是藍鑽收購，「收」是金幣收購；兩種 NPC 可以同一個安全區同時出現。',
    ...
    {
        key: 'system', kw: ['刪除角色', '刪角', '匯出', '匯入', '斷線', '登出', '快取', '舊圖', '重新整理', '換角色', '多開', '離線', '備份', '存檔不見', '角色不見', '倉庫', '搜尋', '藍鑽', '藍鑽', '寵物還原', '傭兵還原', 'file', 'file://', '記憶體'],
        gen: function () {
            return [
                ...
                '匯出會帶角色、倉庫跟藍鑽；匯入時照選項還原倉庫與寵物資料，隊伍狀態會整理避免跨角色出戰錯亂。',
...
const WC_GAME_CONTEXT_WORDS = [
    ...
    '怪物', '魔物', '頭目', '掉落', '掉寶', '地圖', '練功', '掛機', '等級', '經驗', '金幣', '藍鑽',
