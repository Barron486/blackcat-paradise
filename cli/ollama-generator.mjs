...
export async function generateOllamaChat({prompt,model,baseUrl,signal,fetchImpl=fetch,timeoutMs=150000}={}){
  ...
  try{
    const data=await readJson(await fetchImpl(base+'/api/chat',{method:'POST',redirect:'error',headers:{'Content-Type':'application/json'},signal:AbortSignal.any([AbortSignal.timeout(Math.min(150000,Math.max(1,timeoutMs))),...(signal?[signal]:[])]),body:JSON.stringify({model,messages:[{role:'user',content:prompt}],stream:false,think:false,format:{type:'object',properties:{text:{type:'string'}},required:['text'],additionalProperties:false},options:{num_predict:256,temperature:0.8},keep_alive:'5m'})}));
    ...
    if(typeof value?.text!=='string'||!value.text.trim()||value.text.length>240||Object.keys(value).some(k=>k!=='text'))throw failure('GENERATION_FAILED');
