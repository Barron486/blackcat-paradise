...
export async function once({codexBin,fetchImpl=fetch,ollamaFetchImpl=fetch,signal,generateImpl=generateChat,generateOllamaImpl=generateOllamaChat}={}){
  ...
  try{
    ...
    const completion=await request(c.serverUrl,c.token,'/api/ai-chat/complete',{jobId:job.id,leaseToken:job.leaseToken,text:result.text,usage:result.usage},fetchImpl);return {jobId:job.id,completed:!completion.skipped,skipped:!!completion.skipped,provider};
