...
(() => {
    ...
    root().innerHTML = `<div class="ai-chat-grid">
      <section class="surface ai-chat-main">
        ...
        <div class="ai-form-grid">
          <label class="toggle-label"><input id="ai-enabled" type="checkbox"><span><strong>啟用 AI 自動聊天</strong><small>依對話內容與角色個性接話</small></span></label>
          <label class="toggle-label"><input id="ai-respond" type="checkbox"><span><strong>回覆真人玩家</strong><small>看到玩家訊息時可接話</small></span></label>
          <label class="toggle-label"><input id="ai-ambient" type="checkbox"><span><strong>聊天室安靜時主動閒聊</strong><small>預設關閉；開啟後依間隔嘗試接續話題</small></span></label>
        ...
        <fieldset class="speaker-field"><legend>發言角色與個性</legend><p class="hint">可勾選多位一般玩家角色；全部角色共用發言間隔，僅在線角色會接話。被點名時優先回覆，角色資料保持私密。</p><p id="ai-speaker-count" class="field-note" role="status"></p><div id="ai-speakers" class="speaker-list"></div></fieldset>
  ...
  function fillSettings(data) {
    ...
    $('ai-enabled').checked = !!s.enabled; $('ai-respond').checked = s.respondToPlayers !== false; $('ai-ambient').checked = !!s.ambientChat;
    ...
    (opts.speakers || []).forEach(p => {
      const card = document.createElement('div'); card.className = 'speaker-card';
      const label = document.createElement('label'); label.className = 'speaker-option';
      const input = document.createElement('input'); input.type = 'checkbox'; input.value = p.id; input.checked = selected.has(p.id);
      const name = document.createElement('span'); name.textContent = `${p.name || p.username}（${p.username}） · ${p.cls || '玩家'}${p.online ? ' · 在線' : ''}`;
      label.append(input, name);
      const editor = document.createElement('label'); editor.className = 'speaker-personality'; editor.hidden = !input.checked; editor.textContent = '角色個性（最多 500 字，留白使用預設）';
      const persona = document.createElement('textarea'); persona.rows = 3; persona.maxLength = 500; persona.dataset.speaker = p.id; persona.value = s.personas?.[p.id] || ''; persona.placeholder = p.defaultPersonality || '依職業使用預設個性'; persona.addEventListener('input', markDirty);
      editor.append(persona); card.append(label, editor); $('ai-speakers').append(card);
      input.addEventListener('change', () => { editor.hidden = !input.checked; updateSpeakerCount(); markDirty(); });
    });
    updateSpeakerCount();
  ...
  function updateSpeakerCount() { $('ai-speaker-count').textContent = `已選 ${root().querySelectorAll('#ai-speakers input:checked').length} 位角色`; }
  ...
  function values() { const fields = [$('ai-interval'), $('ai-limit')]; if($('ai-provider').value==='ollama')fields.push($('ai-ollama-url'));if (fields.some(field => !field.reportValidity())) throw new Error('請輸入有效的設定值'); return { enabled: $('ai-enabled').checked, provider: $('ai-provider').value, model: $('ai-provider').selectedOptions[0]?.dataset.model || '',ollamaUrl:$('ai-ollama-url').value.trim(),ollamaModel:$('ai-ollama-model').value.trim(), topic: $('ai-topic').value, customPrompt: $('ai-custom').value, intervalSeconds: Number($('ai-interval').value), dailyMessageLimit: Number($('ai-limit').value), respondToPlayers: $('ai-respond').checked, ambientChat: $('ai-ambient').checked, personas: Object.fromEntries([...root().querySelectorAll('#ai-speakers textarea')].map(el => [el.dataset.speaker, el.value])), speakers: [...root().querySelectorAll('#ai-speakers input:checked')].map(i => i.value), revision: latest?.settings?.revision }; }
