...
window.initWorldAdmin=function({api,notify}){
  ...
  async function load(){
    ...
    for(const r of data.history){
      ...
      const subject=c.type==='global'?'世界倍率／顯示設定':c.type==='broadcast'?'稀有掉落廣播':c.type==='map'?'地圖：'+(maps.find(m=>m.id===c.mapId)?.name||c.mapId):'掉落：'+JSON.parse(c.key).slice(1).join(' / ');
      const detail=c.type==='broadcast'?`${c.enabled?'開啟':'關閉'} · ${c.rarities.map(r=>({legend:'傳說',relic:'遺物',rare:'極低掉率'})[r]).join('、')||'未選分類'}`:c.type==='drop'?(c.rate===null?'恢復原始':fmt(c.rate)):c.type==='map'?`${c.open?'開放':'關閉'} · Lv.${c.minLevel}`:`金幣 ×${c.goldMultiplier} / 經驗 ×${c.expMultiplier} / 掉落 ×${c.dropMultiplier}`;
  ...
  async function update(command){
    ...
    const result=await api('/api/gm/world-settings',pending.body);settings=result.settings;pending=null;notify(command.type==='broadcast'?'廣播設定已儲存；玩家畫面約 4 秒更新。':'世界設定已儲存；在線角色於下次同步套用。');
  }
