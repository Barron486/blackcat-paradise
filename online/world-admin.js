...
window.initWorldAdmin=function({api,notify}){
  ...
  $('view-world-settings').innerHTML=`<section class="surface"><div class="section-heading"><h2>世界倍率</h2><button id="world-refresh" class="secondary small">重新整理設定</button></div>
    ...
    <section class="surface world-section"><h2>攻城管理</h2><form id="world-siege"><p class="hint">分別控制各城是否接受新的宣戰。關閉不會中止已開始的攻城，也不影響既有城堡所有權。</p><div class="world-fields"><label class="world-check"><input id="world-siege-kent" type="checkbox">開放肯特城攻城</label><label class="world-check"><input id="world-siege-windwood" type="checkbox">開放風木城攻城</label><label class="world-check"><input id="world-siege-heine" type="checkbox">開放海音城攻城</label></div><label>修改原因<input id="world-siege-reason" minlength="2" maxlength="200" required placeholder="例如：暫停海音城攻城活動"></label><button class="primary" type="submit">儲存攻城設定</button></form></section>
  ...
  async function load(){
    ...
    const siege=settings.siege||{kent:true,windwood:true,heine:true};for(const city of ['kent','windwood','heine'])$('world-siege-'+city).checked=siege[city]!==false;
    ...
    for(const r of data.history){
      ...
      const labels={monsters:'怪物能力／擊殺廣播','monster-strength':'全服怪物強度','kill-broadcast':'擊殺廣播設定',announcement:'GM 文字廣播','announcement-clear':'關閉 GM 廣播',siege:'攻城管理'};
      ...
      const detail=labels[c.type]?(c.type==='monsters'?`${c.ids.length} 種怪物${c.reset?' · 恢復原始能力':''}`:c.type==='monster-strength'?`HP／傷害 ×${c.strength}`:c.type==='kill-broadcast'?`${c.enabled?'開啟':'關閉'} · ${c.monsters.length} 種怪物`:c.type==='announcement'?`${c.pinned?'置頂':'跑馬燈'} · ${c.seconds} 秒 · ${c.text}`:c.type==='siege'?Object.entries(c.castles).map(([city,open])=>`${({kent:'肯特',windwood:'風木',heine:'海音'})[city]}：${open?'開':'關'}`).join(' · '):'提前關閉'):c.type==='presence'?(c.showPlayerLocations?'向所有玩家公開':'限自己、GM 與同血盟成員'):c.type==='broadcast'?`${c.enabled?'開啟':'關閉'} · ${c.rarities.map(r=>({legend:'傳說',relic:'遺物',rare:'極低掉率'})[r]).join('、')||'未選分類'}`:c.type==='drop'?(c.rate===null?'恢復原始':fmt(c.rate)):c.type==='map'?`${c.open?'開放':'關閉'} · Lv.${c.minLevel}`:`金幣 ×${c.goldMultiplier} / 經驗 ×${c.expMultiplier} / 掉落 ×${c.dropMultiplier}`;
  ...
  $('world-siege').onsubmit=event=>{event.preventDefault();const button=event.submitter;button.disabled=true;run(async()=>{await update({type:'siege',castles:Object.fromEntries(['kent','windwood','heine'].map(city=>[city,$('world-siege-'+city).checked])),reason:$('world-siege-reason').value});await load();}).finally(()=>button.disabled=false);};
