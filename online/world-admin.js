...
window.initWorldAdmin=function({api,notify}){
  ...
  let settings, maps=[],dropPage=1,dropRequest=0,activeDrop=null,pending=null;
  ...
  async function load(){
    ...
    for(const r of data.history){const c=r.command,entry=make('article',null,'audit-entry'),text=make('div');let subject=c.type==='global'?'世界倍率／顯示設定':c.type==='map'?'地圖：'+(maps.find(m=>m.id===c.mapId)?.name||c.mapId):'掉落：'+JSON.parse(c.key).slice(1).join(' / ');text.append(make('strong',subject),make('p',c.reason),make('small',r.actor+' · '+(c.type==='drop'?(c.rate===null?'恢復原始':fmt(c.rate)):c.type==='map'?`${c.open?'開放':'關閉'} · Lv.${c.minLevel}`:`金幣 ×${c.goldMultiplier} / 經驗 ×${c.expMultiplier} / 掉落 ×${c.dropMultiplier}`)));entry.append(text,make('time',new Date(r.created_at).toLocaleString('zh-TW')));$('world-history').append(entry);}
  ...
  async function update(command){
    ...
    const result=await api('/api/gm/world-settings',pending.body);settings=result.settings;pending=null;notify('世界設定已儲存；在線角色於下次同步套用。');
  }
  ...
  $('world-refresh').onclick=()=>{pending=null;run(load);};$('world-map-search').oninput=renderMaps;
