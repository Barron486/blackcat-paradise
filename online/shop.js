...
if(cloud){
  ...
  function showSpyReport(report){
    ...
    const abilitiesTitle=document.createElement('h5');abilitiesTitle.textContent='角色能力值';target.append(abilitiesTitle);
    const abilities=document.createElement('dl');abilities.className='diamond-spy-stats diamond-spy-abilities';
    for(const [label,value,suffix] of [['力量 STR',report.attributes?.str],['敏捷 DEX',report.attributes?.dex],['體質 CON',report.attributes?.con],['智力 INT',report.attributes?.int],['精神 WIS',report.attributes?.wis],['魅力 CHA',report.attributes?.cha],['防禦 AC',report.defenses?.ac],['魔法防禦 MR',report.defenses?.mr,'%']]){
      const dt=document.createElement('dt'),dd=document.createElement('dd');dt.textContent=label;dd.textContent=typeof value==='number'&&Number.isFinite(value)?String(value)+(suffix||''):'—';abilities.append(dt,dd);
    }target.append(abilities);
